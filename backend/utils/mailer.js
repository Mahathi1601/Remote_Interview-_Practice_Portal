...
const sendOTPEmail = async (email, otp) => {
    ...
    // 🌟 Brevo HTTP API Mode (Highly recommended for Render Free tier - allows sending to any email address)
    if (process.env.BREVO_API_KEY) {
        try {
            ...
            const response = await fetch('https://api.brevo.com/v3/smtp/email', {
                ...
                body: JSON.stringify({
                    sender: { name: 'Interview Portal', email: process.env.SMTP_USER || 'mahasabbani16@gmail.com' }, // Must be your verified Brevo sender email
                    to: [{ email: email }],
